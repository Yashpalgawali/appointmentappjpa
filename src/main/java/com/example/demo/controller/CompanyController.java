...
public class CompanyController {
	...
	public ResponseEntity<ResponseDto> createCompany(@Valid @RequestBody Company company) {
		...
		return ResponseEntity.status(HttpStatus.CREATED).body(new ResponseDto(company.getCompanyName()+" is created successfully",HttpStatus.CREATED.toString()));
	}
	
	@PutMapping("/")
	public ResponseEntity<ResponseDto> updateCompany(@Valid @RequestBody Company company) {
		compserv.updateCompany(company);
		return ResponseEntity.status(HttpStatus.CREATED).body(new ResponseDto(company.getCompanyName()+" is updated successfully",HttpStatus.CREATED.toString()));
	}
